...
DEFECT_CLASSES = ['punching_hole', 'welding_line', 'crescent_gap', 'water_spot', 'oil_spot', 
    ...
    try:
        ...
        class GC10MultiOutputNet(nn.Module):
            def __init__(self, num_classes):
                ...
                self.cls_head = nn.Sequential(nn.Linear(in_feat, 1024), nn.ReLU(), nn.Dropout(0.5),
                    nn.Linear(1024, 512),     nn.ReLU(), nn.Dropout(0.3),
                    nn.Linear(512, num_classes))
                self.bbox_head = nn.Sequential(nn.Linear(in_feat, 1024), nn.ReLU(), nn.Dropout(0.5),
...
def render_bar(score, color_hex):
    ...
    st.markdown(f'<div class="bar-bg">'
...
def classify_image(pil_img, ood_threshold):
    ...
    all_probs = sorted([{'label': CLASS_NAMES[i], 'score': float(p)} for i, p in enumerate(probs)],
        key=lambda x: -x['score'])

    return {'top_label': top_label, 'top_score': top_score, 'all_probs': all_probs,
        'cls_bbox':  [x1, y1, x2, y2], 'ood_score': ood_score, 'is_ood': is_ood}
...
def detect_with_yolo(pil_img, thresh):
    ...
    for r in results:
        ...
        for box in r.boxes:
            ...
            detections.append({'label': label, 'label_pt': CLASS_PT.get(label, label),
...
def render_resultado_classificacao(cls_result, confiante, limiar_cls, ood_threshold):
    ...
    if cls_result.get('is_ood'):
        ...
        st.markdown(f'<div class="card-info">'
            ...
            f'<div style="margin-top:0.5rem;">', unsafe_allow_html=True)
        render_bar(top_score, '#1c5d99')
        st.markdown(f'<div style="margin-top:0.4rem;font-size:1.3rem;font-weight:700;">'
            f'{top_score*100:.1f}%</div></div></div>', unsafe_allow_html=True)
    else:
        st.markdown(f'<div class="card-info">'
        ...
        st.markdown(f'<div style="margin-top:0.4rem;font-size:1.3rem;font-weight:700;">{top_score*100:.1f}%</div>'
...
def render_resultado_deteccao(cls_result, confiante, yolo_dets, limiar_det):
    if cls_result.get('is_ood'):
        ...
        st.markdown('<div class="card-info"><div class="small-muted">Detecção</div>'
        ...
        st.markdown(f'<div class="card-info"><div class="small-muted">Detecção</div>'
        ...
        st.markdown(f'<div style="margin-top:0.4rem;font-size:1.3rem;font-weight:700;">'
            f'{best["score"]*100:.1f}%</div></div></div>', unsafe_allow_html=True)
...
def render_resultado_visual(nome, imagem, cls_result, confiante, yolo_dets,
    ...
    with col_img:
        ...
        if confiante and YOLO_OK and yolo_dets:
            ...
            st.markdown('<div class="result-caption">Com marcações</div>', unsafe_allow_html=True)
        else:
    ...
    with col_det:
        if not cls_result.get('is_ood'):
            st.markdown('<div class="section-title">Detecção</div>', unsafe_allow_html=True)
...
with aba_analise:
    if not CLS_OK:
        st.error(f"Modelo de classificação não carregado. "
            f"{'Erro: ' + cls_err if cls_err else 'Coloque gc10det_cls_ood.pt na pasta do app.'}")
        st.stop()
    if not YOLO_OK:
        st.warning(f"Modelo YOLO não carregado — detecção com bbox desabilitada. "
            f"{'Erro: ' + yolo_err if yolo_err else 'Coloque yolo_detector.pt na pasta do app.'}")
    if not OOD_ATIVO:
        st.warning("⚠️ Modelo com 10 classes (sem OOD). "
    ...
    with cfg1:
        limiar_cls_pct = st.number_input("Confiança mínima — classificação (%)", 1, 99, 40, 1,
            help="Abaixo deste valor o resultado é inconclusivo e o YOLO não roda.")
    with cfg2:
        limiar_det_pct = st.number_input("Confiança mínima — detecção (%)", 1, 99, 25, 1,
            help="Detecções do YOLO abaixo deste valor são descartadas.")
    with cfg3:
        ood_default = int(det_meta.get('ood_threshold', 0.40) * 100)
        ood_pct = st.number_input("Limiar OOD (%)", 1, 99, ood_default, 5,
    ...
    with col_upload:
        arquivos = st.file_uploader("Envie imagens ou vídeo para análise", type=IMAGE_EXTS + VIDEO_EXTS,
            accept_multiple_files=True, key=f'upload_{st.session_state.uploader_key}')
    if not arquivos:
        ...
        with col_preview:
            ...
            for arquivo in arquivos:
                ...
                st.markdown(f'<div style="font-size:0.85rem;color:#7a90a4;margin-bottom:0.3rem;">{arquivo.name}</div>',
                    unsafe_allow_html=True)
            ...
            with st.spinner("Executando análise..."):
                for arquivo in arquivos:
                    ...
                    if ext in IMAGE_EXTS:
                        ...
                        analise = analisar_pipeline_imagem(imagem, limiar_cls, limiar_det, ood_threshold)
                        resultados.append({'tipo': 'imagem', 'nome': arquivo.name, 'imagem': imagem.copy(), **analise})

                    elif ext in VIDEO_EXTS:
                        frames = extrair_frames_1fps(arquivo.read(), arquivo.name, int(max_frames_video))
                        frames_res = []
                        for item in frames:
                        ...
                        for fr in frames_res:
                            if fr['cls'].get('is_ood'):
                                ...
                                resumo[CLASS_PT.get(fr['cls']['top_label'], fr['cls']['top_label'])] += 1
                        resultados.append({'tipo': 'video', 'nome': arquivo.name, 'frames': frames_res, 'total_frames': len(frames_res),
    ...
    if arquivos and st.session_state.resultado is not None:
        for item in st.session_state.resultado:
            if item['tipo'] == 'imagem':
                render_resultado_visual(nome=item['nome'], imagem=item['imagem'],
                    cls_result=item['cls'], confiante=item['confiante'], yolo_dets=item['yolo_dets'],
                    limiar_cls=limiar_cls, limiar_det=limiar_det, ood_threshold=ood_threshold)
                ...
                if item['resumo_classes']:
                    df_r = pd.DataFrame([{'Classe': k, 'Frames': v} for k, v in item['resumo_classes'].items()])
                    st.dataframe(df_r, use_container_width=True, hide_index=True)

                frames_com = [fr for fr in item['frames'] if fr['yolo_dets']]
                st.markdown('## Resultados da análise')
                if not frames_com:
                    st.info("Nenhum frame com detecção acima do limiar.")
                else:
                #     for fr in frames_com:
                #         render_resultado_visual(nome=f"{item['nome']} — segundo {fr['segundo']}",
                #             imagem=fr['imagem'], cls_result=fr['cls'], confiante=fr['confiante'], yolo_dets=fr['yolo_dets'],
                #             limiar_cls=limiar_cls, limiar_det=limiar_det, ood_threshold=ood_threshold,
    ...
    **Pipeline deste app:**
    ...
    for i, cls in enumerate(DEFECT_CLASSES):
        with (c1 if i % 2 == 0 else c2):
            st.markdown(f'<div class="card-soft" style="margin-bottom:0.85rem;">'
    ...
    dist_data = { 'silk_spot': 650, 'water_spot': 289, 'welding_line': 273,
        'crescent_gap': 226, 'punching_hole': 219, 'inclusion': 216,
        'oil_spot': 204, 'waist_folding': 146, 'crease': 52, 'rolled_pit': 31}
    df_dist = pd.DataFrame([{'Classe': k, 'Português': CLASS_PT.get(k, k), 'Imagens': v} for k, v in dist_data.items()])
...
with aba_ex_imagens:
    ...
    for i, (label, path) in enumerate(exemplos_video):
        with cols_v[i]:
            ...
            if path.exists():
                st.video(str(path))
                st.download_button("Baixar", path.read_bytes(), file_name=path.name, mime="video/mp4", key=f"dl_vid_{i}")
            else:
